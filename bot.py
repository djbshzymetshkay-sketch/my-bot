...
from pyxt.futures import XT_Futures  # استفاده از پکیج رسمی صرافی
...
# راه‌اندازی کلاینت رسمی فیوچرز XT
xt_client = XT_Futures(api_key=XT_API_KEY, secret_key=XT_SECRET_KEY)
...
def test_xt_connection():
    try:
        res = xt_client.get_account_balance()
        if res and (res.get("returnCode") == 0 or res.get("code") == 0 or "result" in res):
...
def get_account_balance_details():
    try:
        ...
        if res:
            ...
            balance = data.get("balance") or data.get("accountWalletBalance") or "0.0"
            available = data.get("availableBalance") or data.get("accountAvailableBalance") or "0.0"
            return f"📊 وضعیت کیف پول فیوچرز صرافی XT:\n\nموجودی کل: {balance} USDT\nموجودی قابل استفاده: {available} USDT"
        return f"خطا در دریافت موجودی: {res}"
    except Exception as e:
        return f"خطای سیستم در دریافت موجودی: {str(e)}"

def advanced_candlestick_and_market_analysis():
    try:
        klines = xt_client.get_kline(symbol="btc_usdt", interval="15m", limit=30)
        if not klines or "result" not in klines:
            return {"status": "error", "message": "خطای دریافت کندل از صرافی"}
            
        candles = klines["result"]
    ...
    while True:
        try:
            ...
            if analysis.get("status") == "success":
                ...
                try:
                    xt_client.set_leverage(symbol="btc_usdt", leverage=applied_leverage, positionSide=position_side)
                    
                    bal_res = xt_client.get_account_balance()
                    ...
                    res_order = xt_client.create_order(
                        symbol="btc_usdt",
                        orderType="1",
                        entrustType="1",
                        bizType="1",
                    ...
                    if res_order and (res_order.get("returnCode") == 0 or res_order.get("code") == 0):
                        success_order = True
                    else:
                        success_order = False
